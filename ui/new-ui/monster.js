...
function setup() {
  ...
  currentState  = deepCopy(DEFAULT_STATE);
  targetParams  = stateToParams(currentState);
...
// Apply a preset: update state + sliders + animate
function setPreset(name) {
  Object.assign(currentState, STATE_PRESETS[name]);
  updateSliders();
  document.querySelectorAll('#presets button').forEach(btn =>
    btn.classList.toggle('active', btn.dataset.preset === name));
  animateTo(stateToParams(currentState));
}

// Sync slider elements to currentState values
function updateSliders() {
  for (const key of Object.keys(DEFAULT_STATE)) {
    const el = document.getElementById('sl-' + key);
    if (el) {
      el.value = currentState[key];
      const valEl = document.getElementById('sv-' + key);
      if (valEl) valEl.textContent = Math.round(currentState[key]);
    }
  }
}

// Build slider panel + preset buttons
function buildSliders() {
  // Preset buttons
  const presetWrap = document.getElementById('presets');
  for (const name of Object.keys(STATE_PRESETS)) {
    const btn = document.createElement('button');
    btn.textContent = name;
    btn.dataset.preset = name;
    if (name === 'baseline') btn.classList.add('active');
  ...
  for (const group of SLIDER_GROUPS) {
    ...
    for (const key of group.keys) {
      ...
      valDisplay.id = 'sv-' + key;
      valDisplay.textContent = currentState[key];

      slider.addEventListener('input', () => {
        currentState[key] = Number(slider.value);
        valDisplay.textContent = slider.value;
        // Clear preset highlight since we're in manual mode
        document.querySelectorAll('#presets button').forEach(b => b.classList.remove('active'));
        // Direct update (no animation for per-slider tweaks)
        targetParams  = stateToParams(currentState);
        currentParams = deepCopy(targetParams);
        fromParams    = deepCopy(targetParams);
        animT = 1.0;
